init python:
    def screenshot_srf():
        # Ren'Py 8.5 GL2 dropped screenshot()'s second (fullscreen) argument
...
init python:
    ...
    class Tear(renpy.Displayable):
        def __init__(self, number, offtimeMult, ontimeMult, offsetMin, offsetMax, srf=None):
            ...
            # Py3: true division above yields floats; sizes must be ints
            self.width = int(self.width)
...
image m_rectstatic:
    ...
    size (32, 32)
image m_rectstatic2:
    RectStatic(Transform(Crop((100, 100, 128, 128), "gui/logo.png"), zoom=0.25), 2, 32, 32).sm
image m_rectstatic3:
    RectStatic(Transform(Crop((100, 100, 64, 64), "gui/menu_art_s.png"), zoom=0.5), 2, 32, 32).sm
...
init python:
    ...
    class AnimatedMask(renpy.Displayable):
        ...
        def render(self, width, height, st, at):
            ...
            rv.operation_complete = (
                self.oc
                + math.pow(math.sin(st * self.speed / 8), 64 * self.frequency) * self.amount
            )
...
image blood_particle_drip:
    "gui/blood_drop.png"
    subpixel True yanchor 0.2 yzoom 0
    linear 10 yzoom 8
